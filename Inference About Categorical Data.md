...
for(i in 1:nP){
  ...
  int[i,,]  <- conInt
  cov[i, 1] <- mean((conInt[, 1] + noi  < p[i]) & (conInt[, 2] + noi > p[i]))
  cov[i, 2] <- mean((conInt[, 3] + noi  < p[i]) & (conInt[, 4] + noi > p[i]))
...
plot(x    = p,
     ...
     type = 'l',
     xlab = "Population Probability",
     ylab = "Simulated Interval Lenght",
     lwd  = 2,
     col = rgb(1, 0, 0, 0.5))
par(new=TRUE)
plot(x    = p,
...
When the sample size is large enough, the test statistic is approximately standard normal under the null hypothesis:

$$
z \approx N(0,1).
$$

This approximation is justified by the large-sample behavior of the sample proportion.

A common condition is that the expected numbers of successes and failures under the null hypothesis are both sufficiently large, for example:
...
Some instructors or books use 10 instead of 5 as a more conservative guideline.

These conditions matter because if the expected counts are too small, the normal approximation may be poor, and an exact binomial test may be more appropriate.

---

#### Decision Rule and p-Value

Once the test statistic is computed, there are two equivalent ways to proceed:

- compare the value of $z$ to the appropriate critical value
- compute the p-value
...
the p-value is the probability, assuming $H_0$ is true, of observing a value of the test statistic at least as far from 0 as the one obtained.
...
A small p-value indicates that the observed sample proportion would be unusual if the null hypothesis were true, and therefore provides evidence against $H_0$.
